...
The main design decisions we have not made and is stopping this project from progressing are:

- do we want separate 'back-end' data collectors?
- do we need a 'server' to collect data
- does our data-visualization need to be CPU intensive ?
- could it be browser based? ()
- what platform and UI-library will we use

## Collector -> Server -> Visualizer

One way to go would be to have python backends collecting data, send the data to a central server and have multiple clients.
We could have browser-client and a native-client. The advantage of the server being that you can connect a client later after collection starts. And closing the visualization would not stop data-collection.

## All-in-one

Another way could be to tail a log-file, much like debugview++, the visualization would be done locally on the system that
has the log. Or it would run remote, and receive message by socket.

These setups are very different.
...
- a 1 microsecond accurate storage will cover most usecases. On very time critical systems where time is measured in nanoseconds (1e-9), picoseconds (1e-12) or femtoseconds (1e-15) optimizations are usually measured/aggregated in loops.
...
- online tools (such as google's Perfetto) can (and do) change input format over time and become incompatible with older data (bugs or not, it is no longer ABI compatible), so consider a 'frozen' offline viewer to unsure that very data can be read anytime. Perfetto can be installed locally, so that can be a way to deal with that.
- some trace formats (like VCD) require that you know up front what 'signals' are going to be present, this is problematic, because when 'streaming' logs all possible occurrences of names of function/events are not known up front. So this kind of format requires post-processing.
...
- online filters/highlighting storage to sync settings over difference workstations (login with google account?)
...
- categorization
- summaries (frequency, totals occurrences)
- analysis
- visualization
...
- network messages, listening to a TCP/UDP socket, specifically from multiple systems that work together.
- network traffic, as captured by tcp-dump

Formatting:

- allow to edit columns, on tabs that read form files, maybe apply restrictions on OutputDebugString/Event log tabs.
- unformating/parsing by allowing specification of column separator or fixed width (per column)
- allow/be agnostic to (log)files without timestamps (create timestamps for such input as the input arrives)
...
- http://logio.org/ (looks good, stopped development in 2020)
...
- https://www.scalyr.com/product (website gone...) (tag: Log aggregation, system metrics, and server monitoring that's
...
- https://www.logfusion.ca/Screenshots/ (actually a bit like a debugview++, not optimized for speed, but nicer UI)
...
- http://www.farrellf.com/TelemetryViewer/ (java based, no updates since 2021)
- https://devblogs.microsoft.com/pix/download/ (Performance tuning and debugging for DirectX 12 games on Windows)
