...
class ScopeRenderer {
    constructor(canvas, config) {
        ...
        // make the canvas as big as the browser area
        this.resize();

        // when the window is resized, also resize the canvas
    ...
    render(paused) {
        ...
        if (!paused) {
            this.smoothedNow += (realNow - this.smoothedNow) * this.config.TIME_SMOOTHING;
            this.frozenNow = this.smoothedNow;
        }

        const now = paused ? this.frozenNow : this.smoothedNow;

        if (!paused) this.pruneEvents(now);
